...
// ============================================================================
// 1) Update getFragranceData() — add 30 new entries before the closing };
// ============================================================================

const newFragranceEntries = `,
...
// Find the blv entry in getFragranceData and add after it
const fragranceOldBlv = `      blv: {
        name: 'Man in Black',
        brand: 'Bvlgari',
        price: '$55',
...
if (script.includes(fragranceOldBlv)) {
    script = script.replace(fragranceOldBlv, fragranceNewBlv);
...
// ============================================================================
// 2) Update second getProductDetails() (line ~11746) — simpler version
// ============================================================================

const productDetails2Old = `      blv: { name: "Man in Black", brand: "Bvlgari", image: "bvlgari-man-in-black.png" },
    };
    return products[productId];`;

const newProductEntries = `      blv: { name: "Man in Black", brand: "Bvlgari", image: "bvlgari-man-in-black.png" },
...
if (script.includes(productDetails2Old)) {
    script = script.replace(productDetails2Old, newProductEntries);
    console.log('✓ Updated getProductDetails() #2');
} else {
    console.log('✗ Could not find getProductDetails() #2 blv entry!');
}

// ============================================================================
// 3) Update first getProductDetails() — it has fallback, so no entries needed
// ============================================================================
// The first getProductDetails only has layton/haltane/pegasus with full details.
// Other perfumes use a fallback. This is already working — no change needed.
console.log('✓ getProductDetails() #1 uses fallback — no change needed');

// ============================================================================
// Save
// ============================================================================
fs.writeFileSync('script.js', script);
console.log('✓ Saved script.js (fragrance data + product details updated)');
